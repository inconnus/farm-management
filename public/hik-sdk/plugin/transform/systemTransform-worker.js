...
// 转封装库回调函数
self.STCallBack = (fileIndex, indexLen, data, dataLen) => {
  //stFrameInfo的类型见DETAIL_FRAME_INFO
  const stFrameInfo = Module._GetDetialFrameInfo();
  const nIsMp4Index = stFrameInfo.nIsMp4Index;
    ...
    if (RECORDRTP) {
      ...
      if (iRet != 0) {
        ...
        if (data.options && data.options.pKeyData) {
          ...
          var iRet = Module._SysTransSetEncryptKey(
            secretInfo.nKeyType,
            pKeyData,
            keyLen,
            nKeySize,
