...
var vertexLineShader = [
  '#version 300 es',
  'layout(location = 0) in vec4 vertexPosLine;',
  'void main()',
  '{',
  'gl_Position = vertexPosLine;',
  '}',
].join('\n');
var fragmentLineShader = [
  '#version 300 es',
  'precision highp float;',
  'uniform mediump float fRcom;',
  'uniform mediump float fGcom;',
  'uniform mediump float fBcom;',
  'out vec4 fragColor;',
  'void main()',
  '{',
  'fragColor = vec4(fRcom,fGcom,fBcom,1.0);',
  '}',
].join('\n');
((e, r) => {
  e.SuperRender2 = r();
  ...
  e.prototype.initProgram = function (e, r) {
    ...
    if (!t.getShaderParameter(a, t.COMPILE_STATUS)) {
      console.log('Vertex shader failed to compile: ' + t.getShaderInfoLog(a));
    }
    ...
    if (!t.getProgramParameter(o, t.LINK_STATUS)) {
      console.log('Program failed to compile: ' + t.getProgramInfoLog(o));
    }
    t.deleteShader(a);
    t.deleteShader(i);
    return o;
