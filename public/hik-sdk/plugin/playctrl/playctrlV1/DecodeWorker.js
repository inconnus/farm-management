...
((event) => {
  ...
  onmessage = (event) => {
    ...
    switch (eventData.command) {
      case 'printLog': {
        const downloadFlag = eventData.data;
      ...
      case 'SetPlayPosition': {
        const nFrameNumOrTime = eventData.data;
        const enPosType = eventData.type;
      ...
      case 'GetBMP': {
        ...
        Module.writeArrayToMemory(
          new Uint8Array(pYUVData, 0, nYUVSize),
          pDataYUV,
        );
        ...
        postMessage({ function: 'GetBMP', data: aBmpData, errorCode: res }, [
          aBmpData.buffer,
        ]);
        aBmpData = null;
        if (pDataYUV != null) {
      ...
      case 'GetJPEG': {
        ...
        Module.writeArrayToMemory(
          new Uint8Array(pYUVData1, 0, nYUVSize1),
          pDataYUV1,
        );
        ...
        postMessage({ function: 'GetJPEG', data: aJpegData, errorCode: res }, [
          aJpegData.buffer,
        ]);
      ...
      case 'InputAudEncodeData': {
        ...
        if (iLen > 0) {
          ...
          if (bWorkerPrintLog) {
            console.log(
              '<<<Worker: 20200113 DecodeWorker-InputAudEncodeData 2 res:' +
                res,
            );
          }
          ...
          if (res == PLAYM4_OK) {
            ...
            if (bWorkerPrintLog) {
              console.log(
                '<<<Worker: 20200113 DecodeWorker-InputAudEncodeData 2-1 succ',
              );
            }
            //调用C++ GetAudFrameData
  ...
  function getAudFrameData(fun) {
    ...
    if (res === PLAYM4_OK) {
      ...
      aEncodeAudData.set(
        Module.HEAPU8.subarray(pEncodeAud, pEncodeAud + iSize),
      );
      if (bWorkerPrintLog) {
      ...
      postMessage({
        function: 'GetAudEncodeData',
        data: aEncodeAudData.buffer,
        dataSize: iSize,
        errorCode: res,
      });
    } else {
    ...
    if (res === PLAYM4_OK) {
      ...
      switch (oFrameInfo.frameType) {
        ...
        case VIDEO_TYPE: {
          ...
          if (bWorkerPrintLog) {
            console.log(
              '<<<Worker: InputData-getFrameData Video: Width:' +
                oFrameInfo.width +
                ',Height:' +
                oFrameInfo.height +
                ',timeStamp:' +
      ...
      if (PLAYM4_DECODE_ERROR === res) {
        ...
        aRawData.set(
          Module.HEAPU8.subarray(pRawData, pRawData + rawInfo.isize),
        );
        postMessage({
          function: 'GetRawData',
          type: '',
      ...
      //定位返回的错误
      if (
        PLAYM4_FIRST_FRAME_NOT_ICURRENT === res ||
        PLAYM4_ITYPE_DECODE_ERROR === res
      ) {
        postMessage({
          function: 'GetFrameData',
          type: '',
      ...
      //需要更多数据
      if (
        PLAYM4_NEED_MORE_DATA === res ||
        PLAYM4_SYS_NOT_SUPPORT === res ||
        PLAYM4_NOT_KEYFRAME === res
      ) {
        postMessage({
          function: 'GetFrameData',
          type: '',
