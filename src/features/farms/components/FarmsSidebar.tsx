...
export const FarmsSidebar = () => {
  ...
  const [clickedPolygonLandId, setClickedPolygonLandId] = useAtom(
    clickedPolygonLandIdAtom,
  );
  const { farmId, orgSlug, landId } = useParams<{
    farmId?: string;
    orgSlug?: string;
    landId?: string;
  }>();
  ...
  const sensorTelemetryQueries = useIOTTelemetryQueries(sensorAppIotIds);
      ...
      {farmId &&
        sensors.map((s) => (
          <SensorMarker
            key={s.id}
            sensor={s}
            isOnline={sensorTelemetryMap.get(s.appIotId) ?? false}
            onClick={(sensor) =>
              setDevicePopup({
                type: 'sensor',
