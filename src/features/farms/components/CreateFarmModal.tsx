...
type CreateFarmModalProps = {
  ...
  onSubmit?: (data: { name: string; location: PinLocation }) => void;
...
export const CreateFarmModal = ({
  ...
  // ── Map initialisation (runs whenever the modal opens) ───────────────────
  useEffect(() => {
    if (!isOpen) return;

    // Small delay so the modal DOM is fully painted before initialising mapbox
    const timer = setTimeout(() => {
      ...
      const m = new mapboxgl.Map({
        ...
        center: [100.9925, 15.8700], // Centre of Thailand
        zoom: 5,
        projection: 'mercator',
      });
  ...
  const placePinAt = useCallback(
    (mapInstance: mapboxgl.Map, lng: number, lat: number) => {
      if (markerRef.current) {
        ...
        // Build custom green pin element
        ...
        el.innerHTML = `
          ...
          ">
            <div style="
              ...
              background:#03662c;transform:rotate(-45deg);
          ...
          <Modal.Dialog className="sm:max-w-2xl bg-white text-gray-800 border border-gray-200 shadow-2xl">
            ...
            <div className="flex items-center justify-end gap-3 px-6 pb-6">
              ...
              >
                ...
                {isSubmitting ? 'กำลังสร้าง...' : 'สร้างฟาร์ม'}
              </button>
            </div>
          </Modal.Dialog>
        </Modal.Container>
