...
import { FarmDetailPage } from '@features/farms/components/FarmDetailPage';
import {
    useIOTDevicesQuery,
    useLandsQueries,
...
const LandTypeColor: Record<string, string> = {
    แหล่งน้ำที่ใช้รดพืช: 'rgb(75, 216, 255)', // หรือระบุเป็น "#4bd8ff"
    พื้นที่แปลงปลูก: 'rgb(63, 189, 67)', // หรือระบุเป็น "#3fbd43"
    บ้านที่อยู่อาศัย: 'rgb(99, 87, 238)', // หรือระบุเป็น "#6357ee"
    ที่พักผลผลิต: 'rgb(1, 124, 3)', // หรือระบุเป็น "#017c03"
    ที่เก็บสารเคมี: 'rgb(232, 177, 59)', // หรือระบุเป็น "#e8b13b"
    อื่นๆ: 'rgb(166, 166, 166)', // หรือระบุเป็น "#a6a6a6"
};
...
const DEFAULT_MAP_OVERVIEW = {
    center: [101.4918194, 12.5352438] as [number, number],
    zoom: 5,
    duration: 1000,
};
...
const DashboardScreen = () => {
    ...
    const dashboardPolygons = useMemo(() => {
        ...
        landsQueries.forEach((query) => {
            ...
            if (queryData) {
                // Determine the actual payload, handling cases where it's wrapped in 'data' or is an array
            ...
            <Column className="pointer-events-auto bg-white/85  backdrop-blur-xl m-3 p-3 rounded-3xl border border-gray-200 shadow-xl w-[380px] max-h-[calc(90vh)] overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto min-h-0 pr-2 -mr-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                    {filteredDevices?.length === 0 && (
                        <div className="text-center py-6 text-gray-400 text-sm">ไม่พบอุปกรณ์ที่ค้นหา</div>
                    )}
                        ...
                        return (
                            <React.Fragment key={device._id}>
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-1.5 flex-wrap">
                                        ...
                                        <span className="inline-flex items-center gap-1 bg-[#ff9800]/10 text-[#e65100] rounded-md px-1.5 py-0.5 text-[10px] font-medium">
                                            💧 ความชื้น {telemetry?.sensor_ambient_humid?.toFixed(1) ?? '-'}%
                                        </span>
                                        ...
                                        {/* <span className="inline-flex items-center gap-1 bg-[#ffc107]/10 text-[#f57f17] rounded-md px-1.5 py-0.5 text-[10px] font-medium">
                                            <ZapIcon size={10} />
                                            แรงดัน {(telemetry?.sensor_voltage_v_in ?? telemetry?.sensor_v_in)?.toFixed(1) ?? '-'}V
                                        </span> */}
                                    </div>
                                    ...
                                    <div className="flex items-center gap-1 mt-1 justify-between">
                                        <Row>
                                            ...
                                            <MapPinIcon size={10} className="text-red-400 shrink-0" />
                                ...
                                <MapMarkerMount
                                    ...
                                    openPopupSignal={
                                        openPopupIdSignal.id === device._id
                                            ? openPopupIdSignal.signal
                                            : 0
                                    }
                                    popup={
